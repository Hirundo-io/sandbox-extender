...
describe("Cedar grouping evaluation", () => {
  test("allows matching Cedar policies with JSON arguments", () => {
    expect(
      evaluateCedarGrouping(
        {
          id: "allow-pr-diff",
          policies: {
            allow:
              'permit(principal, action == Action::"codex.unified_exec", resource) when { context.arguments.command like "gh pr diff*" };',
          },
        },
        context,
      ),
  ...
  test("abstains when no policy matches or policy syntax is invalid", () => {
      ...
      evaluateCedarGrouping(
        {
          ...
          policies: {
            allow:
              'permit(principal, action == Action::"codex.unified_exec", resource) when { context.arguments.command like "gh pr view*" };',
          },
        },
        context,
      ),
    ...
    expect(
      evaluateCedarGrouping({ id: "invalid", policies: { invalid: "not Cedar" } }, context),
    ).toBe("abstain");
  });
