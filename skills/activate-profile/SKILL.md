...
5. If the tool reports that the client does not support elicitation and requires a CLI authorization, submit `bun run authorize:mutation -- activate_profile --thread-id <current-host-thread-id> --arguments-json '{"arguments":<activation-arguments>,"profileId":"<profile-id>"}'` through the host's explicit shell-approval path. Wait for it to succeed, then retry the same MCP call before the authorization expires. Do not use this fallback when elicitation was declined, cancelled, or failed for another reason.
