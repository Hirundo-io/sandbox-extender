...
5. For an SSH target, make a private, mode-600 temporary snapshot of `~/.ssh/config`, and use that same snapshot to build the allowlist and for every SSH command. Accept only an exact, literal alias from a positive `Host` entry. Exclude wildcard and negated patterns, and reject a supplied target that is absent from this allowlist. Do not accept `user@host`, IP addresses, options, or other host syntax even if SSH could parse it. Materialize required `Include` files into the snapshot; otherwise mark that target unavailable rather than falling back to mutable SSH configuration. Use noninteractive, bounded connection options and pass the validated alias as one argument after `--` (for example, `ssh -F "$configSnapshot" -o BatchMode=yes -o StrictHostKeyChecking=yes -o ConnectTimeout=10 -- "$host" <fixed-command>`). Keep the remote command fixed rather than interpolating user input. Run the same checks and commands through that connection, one command at a time; stop mutating that host when its SSH connection or a required command fails, then continue with the remaining targets. Remove the snapshot after the target completes.
