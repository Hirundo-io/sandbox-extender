...
6. Verify each successful target with `bun --version`, `codex plugin list`, and `claude plugin list`. Confirm that Bun is available and `sandbox-extender@sandbox-extender` is installed and enabled for both tools. Versions can differ because Codex and Claude Code package the plugin metadata differently; Bun availability, enabled status, and the marketplace source are the completion criteria.
