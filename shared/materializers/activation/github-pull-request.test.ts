...
describe("GitHub pull request activation materializer", () => {
  test("creates a canonical target", () => {
    expect(
      materializeGitHubPullRequestActivation({ pullRequest: 42, repository: "Acme/Example" }),
  ...
  test("fails closed when current pull request lookup fails", () => {
    for (const output of [
      ...
      expect(
        materializeGitHubPullRequestActivation({ workingDirectory: "/workspace" }, () => output),
      ).toBeUndefined();
    }
  });
  ...
  test("writes the executable result and reports invalid input", async () => {
    const output: string[] = [];
    expect(
      await runGitHubPullRequestActivationMaterializer(
        Promise.resolve({ pullRequest: 42, repository: "acme/example" }),
